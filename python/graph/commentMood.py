...
def findRow(id, db, user, password):
    ...
    with con:
        ...
        cur.execute(
            ...
            'JOIN comment ON comment.publication_id = publication.id '
        ...
        labels = []
        if positive > 0:
            values.append((positive/len(comments))*100)
            labels.append("Позитивные")

        if negative > 0:
            values.append((negative/len(comments))*100)
            labels.append("Негативные")

        if neutral > 0:
            values.append((neutral/len(comments))*100)
            labels.append("Нейтральные")

        fig2, ax2 = plt.subplots()
        ax2.pie(values, labels=labels, autopct='%1.1f%%')
        fig2.savefig('/var/www/image/public/graphs/comments/' + id + '_mood.png')
